...
import sorterBuiltins from './common/sorters.js';
 ...
 * @typedef {import('../types/Sorter.d.ts').Sorter} Sorter
...
export class Register {
   ...
   * @param {Sorter} cfg
   */
  static registerSorter(cfg) {
    // this = class
    this.__registerSorter(cfg, this);
  }
  ...
  registerSorter(cfg) {
    // this = instance
    /** @type {typeof Register} */ (this.constructor).__registerSorter(cfg, this);
  }
   ...
   * @param {typeof Register | Register} target
   */
  static __registerSorter(sorter, target) {
    const { name, sorter: sorterFn } = sorter;
    if (typeof name !== 'string')
      throw new Error("Can't register sorter; sorter.name must be a string");
    if (typeof sorterFn !== 'function')
      throw new Error("Can't register sorter; sorter.sorter must be a function");

    // Prevent overriding built-in 'name' sorter
    // 'name' is hardcoded in formattedVariables.js, so it cannot be overridden
    if (name === 'name') {
      throw new Error(
        `Can't register sorter; "name" is a reserved built-in sorter name. ` +
          `Please choose a different name.`,
      );
    }

    this.deleteExistingHook(target, 'sorters', name);
