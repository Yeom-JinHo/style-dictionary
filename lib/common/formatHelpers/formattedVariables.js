 ...
 * @typedef {import('../../../types/Format.d.ts').SortOption} SortOption
 * @typedef {import('../../../types/Format.d.ts').SortItem} SortItem
 ...
 * @param {Hooks} [options.hooks] - Hooks object containing registered sorters.
 * @returns {String}
 * @example
 * ```js
 ...
 *       sort: options.sort, // optional: 'name', registered sorter name, comparator function, or array
...
export default function formattedVariables({
  ...
  /**
   * @param {SortItem} keyOrFn
   * @returns {((a: Token, b: Token) => number) | null}
   */
  const normalize = (keyOrFn) => {
    if (typeof keyOrFn === 'function') return keyOrFn;

    // Built-in 'name' sorter takes priority over any registered sorter with the same name
    if (keyOrFn === 'name') return sortByName;

    // Check if it's a registered sorter hook
    if (typeof keyOrFn === 'string') {
      const registeredSorter = hooks?.sorters?.[keyOrFn];
      if (registeredSorter) {
      ...
      throw new Error(
        `Invalid "sort" option: "${keyOrFn}". ` +
          `Use "name", a registered sorter name, a comparator function, or an array of those.`,
      );
    }
