 ...
 *
 * @typedef {'name' | 'reference'} BuiltInSort
 * @typedef {BuiltInSort | BuiltInSort[] | ((a: Token, b: Token) => number)} SortOption
 */
...
export default function formattedVariables({
  ...
  // create reference sorter only if we need it
  const needsReferenceSorter = outputReferences || sortInputs.includes('reference');
  const byReference = needsReferenceSorter
    ? sortByReference(tokens, { unfilteredTokens: dictionary.unfilteredTokens, usesDtcg })
    : null;

  /**
   * @param {BuiltInSort | ((a: Token, b: Token) => number)} keyOrFn
  ...
  const normalize = (keyOrFn) => {
    ...
    if (keyOrFn === 'reference') return byReference;

    // Fail loudly for invalid values (esp. typos)
    if (typeof keyOrFn === 'string') {
      throw new Error(
        `Invalid "sort" option: "${keyOrFn}". ` +
          `Use "name" | "reference" (or an array of those), or provide a comparator function.`,
      );
    }
  ...
  const requestedSorters = sortInputs.map(normalize).filter(Boolean);
  ...
  // If outputReferences=true, keep reference-safe ordering FIRST (define-before-use safety).
  // The user-provided sort acts as a tie-breaker on top.
  const comparators = outputReferences
    ? [byReference, ...requestedSorters.filter((s) => s !== byReference)].filter(isComparator)
